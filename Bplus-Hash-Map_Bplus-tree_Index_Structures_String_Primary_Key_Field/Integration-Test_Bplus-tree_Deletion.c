    ...
    for (node_size = MINIMUM_NODE_SIZE; node_size <= MAXIMUM_NODE_SIZE; node_size += NODE_SIZE_INCREMENT_FACTOR)
    {
        ...
        for (repetition = 0; repetition < PROCESS_REPETITIONS_PER_NODE_SIZE; repetition++)
        {
            ...
            for (i = 0; i < INPUT_DATA_SIZE - 1; i++)
            {
                ...
                BplusTreeDeleteData(&Bplustree_Index, Primary_Key, &Retrieved_Data, &Get_PrimaryKey_TableD);


                if (Retrieved_Data != NULL)
                {

                    if ( strcmp(Get_PrimaryKey_TableD(Retrieved_Data), Primary_Key)  != 0 )
                    {
                    ...
                    if (!BplusTreeDeleteData(&Bplustree_Index, Primary_Key, &Retrieved_Data, &Get_PrimaryKey_TableD) || Retrieved_Data != NULL)
                    {
                ...
                Retrieved_Data = BplusTreeFastSearchData_ByPrimaryKey( Bplustree_Index,
                                                                       Get_PrimaryKey_TableD(Selected_Data_List.List_HeadNode->RecordData),
                                                                       &Get_PrimaryKey_TableD );

                if ( Retrieved_Data == NULL
                     || strcmp(Get_PrimaryKey_TableD(Retrieved_Data), Get_PrimaryKey_TableD(Selected_Data_List.List_HeadNode->RecordData)) != 0 )
                {
                ...
                while (CurrentNode != NULL)
                {
                    ...
                    Retrieved_Data = NULL;
                    Retrieved_Data = BplusTreeFastSearchData_ByPrimaryKey( Bplustree_Index,
                                                                           Get_PrimaryKey_TableD(CurrentNode->RecordData),
                                                                           &Get_PrimaryKey_TableD );

                    if ( Retrieved_Data == NULL
                         || strcmp(Get_PrimaryKey_TableD(Retrieved_Data), Get_PrimaryKey_TableD(CurrentNode->RecordData)) != 0 )
                    {
                    ...
                    if ( strcmp(Get_PrimaryKey_TableD(CurrentNode->List_PrevNode->RecordData), Get_PrimaryKey_TableD(CurrentNode->RecordData)) >= 0 )
                    {
