...
)
{
    ...
    return BplusTree_Binary_Search(array_stored_elements, RecordDataArray, PrimaryKey, Get_Record_PrimaryKey);
...
)
{
    ...
    for (int i = 0; i < CurrentNode->array_stored_elements; i++)
    {
        ...
        if ( strcmp(PrimaryKey, Get_Record_PrimaryKey(CurrentNode->RecordDataArray[i])) <= 0 )
        {
            ...
            if ( strcmp(PrimaryKey, Get_Record_PrimaryKey(CurrentNode->RecordDataArray[i])) == 0 && CurrentNode->leaf_type)
            {
...
BplusTreeData InitializeBplusTreeData_TableD(
...
)
{
    ...
    NewRecordData->RecordData = (TableD_RecordData)(malloc(sizeof(struct TableD_RecordData)));
    NewRecordData->table_id = 4;

    ((TableD_RecordData)(NewRecordData->RecordData))->Record_PrimaryKey_Size = Record_PrimaryKey_Size;
    ((TableD_RecordData)(NewRecordData->RecordData))->Record_PrimaryKey = (BplusTreePrimaryKey)(malloc(Record_PrimaryKey_Size * sizeof(char)));


    for (int i = 0; i < ((TableD_RecordData)(NewRecordData->RecordData))->Record_PrimaryKey_Size; i++)
    {

        ((TableD_RecordData)(NewRecordData->RecordData))->Record_PrimaryKey[i] = Record_PrimaryKey[i];

    }


    ((TableD_RecordData)(NewRecordData->RecordData))->Field_Data = Field_Data;
...
BplusTreePrimaryKey Get_PrimaryKey_TableD(
...
)
{
    ...
    return ((TableD_RecordData)(Input_RecordData->RecordData))->Record_PrimaryKey;
...
)
{
    ...
    return ((TableD_RecordData)(Input_RecordData->RecordData))->Field_Data;
...
bool Deallocate_BplusTreeRecordData_TableD(
...
)
{
    ...
    free(((TableD_RecordData)(RecordData_ToBe_Deleted->RecordData))->Record_PrimaryKey);
    free(((TableD_RecordData)(RecordData_ToBe_Deleted->RecordData)));
...
)
{
    ...
    if ( (record_position < CurrentNode->array_stored_elements)
         && ( strcmp(Get_Record_PrimaryKey(CurrentNode->RecordDataArray[record_position]), Record_PrimaryKey) == 0 ) )
    {
...
)
{
    ...
    if (BplusTree->Root->leaf_type)
    {
        ...
        if ( (record_position < BplusTree->Root->array_stored_elements)
             && ( strcmp(Get_Record_PrimaryKey(BplusTree->Root->RecordDataArray[record_position]), PrimaryKey) == 0 ) )
        {
        ...
        //printf("BplusTreePrimaryKey : %s not found\n\n", PrimaryKey);
...
)
{
    ...
    //printf("BplusTree PrimaryKey : %s not found\n\n", PrimaryKey);
